...
You'll first have to either build the program following the instructions [here](https://github.com/that-hatter/scrapi-searcher?tab=readme-ov-file#building), or, if you plan to use Docker, pull one of the images hosted [here](https://github.com/that-hatter/scrapi-searcher/pkgs/container/scrapi-searcher). The following command pulls the latest image:
...
The bot uses custom emojis for certain visuals such as attribute and monster type icons.
These need to be uploaded per bot account (in your Discord Application's settings).
